...
class DualLogisticRegression_Head(nn.Module):
    ...
    def forward(self, x):
        with torch.amp.autocast("cuda", enabled=False):
            ...
            with torch.compiler.disable(recursive=True): # don't compile for now, type mismatch error in torch 2.3.1 in if-else statement
                if self.training:
                    propensity_inv = 1
                else:
                    with torch.no_grad():
...
def adjust_labels(logits, labels, dist_tracker, clip_dist = 0.90, clip_logit = 0.65, eps=1e-8):
    # use a z test
