...
class DualLogisticRegression(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.set_grad_enabled(True):
            ...
            #x = torch.special.logit(propensity / (1+(self.estimator(x.detach())**2) + torch.exp(-self.fc(x)) + self.eps))
            ...
            x = torch.special.logit(self.fc(x).sigmoid() * self.estimator(x).sigmoid())
        return x
