            ...
            for i, (images, tags) in loaderIterable:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    with torch.cuda.amp.autocast(enabled=FLAGS['use_AMP']):
                        ...
                        loss += 30*(((dist_tracker.pos_mean + dist_tracker.neg_mean) / 2) ** 2).sum() #+ dist_tracker.pos_std.sum() + dist_tracker.neg_std.sum()
                        loss -= 30*((dist_tracker.pos_mean - dist_tracker.neg_mean) / ((dist_tracker.pos_var + dist_tracker.neg_var) ** 0.5 + 1e-8)).sum()
